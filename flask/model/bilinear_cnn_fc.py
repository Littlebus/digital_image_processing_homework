...
import numpy as np
import PIL.Image
from model.generate_class_list import generate_class
...
class BCNN(torch.nn.Module):
    ...
    def get_normalize_feature(self,X):
        N = X.size()[0]
        assert X.size() == (N, 3, 448, 448)
        X = self.features(X)
        assert X.size() == (N, 512, 28, 28)
    ...
    def get_fc_feature(self,X):
        return self.forward(X)
    
    def get_softmax_feature(self,X):
        score = self.forward(X)
        #assert score.size() == (200,),score.size()
...
class BCNNManager(object):
    ...
    """
    def __init__(self, options, path, test=False):
    ...
    def norm_feature(self,X):
        return np.squeeze(self._net.module.get_normalize_feature(X).data.cpu().numpy())
    
    def fc_feature(self,X):
        return np.squeeze(self._net.module.get_fc_feature(X).data.cpu().numpy())

    def softmax_feature(self,X):
        return np.squeeze(self._net.module.get_softmax_feature(X).data.cpu().numpy())
...
def prepare_trained_model(model_path,test=True):
    base_lr = 0.05
    batch_size = 32
    epochs = 100
    weight_decay = 5e-4
    ...
    #for d in path:
        #assert os.path.isdir(path[d]),path[d]

    manager = BCNNManager(options, path,test=test)
    manager.load_checkpoint(model_path)

    class_list = generate_class('../data/cub200/raw/CUB_200_2011/classes.txt')
    return manager, class_list

def wrap_image(img_path):
    test_transforms = torchvision.transforms.Compose([
            torchvision.transforms.Resize(size=448),
            torchvision.transforms.CenterCrop(size=448),
            torchvision.transforms.ToTensor(),
    ...
    #image = image.resize((448,448),PIL.Image.BILINEAR)
    image_np = np.array(image)
    image.close()
    img = PIL.Image.fromarray(image_np)
    X = test_transforms(img)
    X = torch.unsqueeze(X,0)
    #image_np = image_np.reshape(3,448,448)
    #X = torch.autograd.Variable(torch.from_numpy(np.array([image_np],dtype=np.float32)).cuda())
    #X = torch.from_numpy(np.array([image_np],dtype=np.float32)).cuda()
    #print(X.size())
    return X.cuda()
def classify_image(manager,img_path):
    X = wrap_image(img_path)
    score = manager._net(X)
